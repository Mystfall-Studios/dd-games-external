 ...
 *   - Ably script tag already loaded (ably.min-1.js)
 *   - localStorage "device_id" set
 *   - Supabase anon key accessible
 */
...
if (!myID || myID === GUEST_ID) throw new Error("Friends panel: guest/no user");

// ─── Detect current game from page title / path ───────────────────────────
...
#fp-header-btns { display: flex; gap: 6px; }
...
/* quick message input inside panel */
#fp-dm-bar {
  display: none;
  gap: 7px;
  padding: 8px 12px;
...
let friends   = [];   // accepted friendships + user data
let pendingIn = [];   // incoming requests
let notifs    = [];   // notifications
let presence  = {};   // { userID: { current_game, last_seen } }
let dmTargetID   = null;
...
function isOnline(uid) {
  ...
  if (!p) return false;
  return Date.now() - new Date(p.last_seen).getTime() < 3 * 60 * 1000; // 3 min window
}
...
function renderFriends() {
  ...
  let html = "";
  if (online.length) {
    html += `<div class="fp-section-label">Online — ${online.length}</div>`;
    online.forEach(f => { html += friendCard(f, true); });
  }
  if (offline.length) {
    html += `<div class="fp-section-label">Offline</div>`;
    offline.forEach(f => { html += friendCard(f, false); });
  }
  pane.innerHTML = html;
...
function friendCard(f, online) {
  ...
  <div class="fp-friend">
    ...
    <div class="fp-friend-actions">
      ...
      <button class="fp-action-btn reject" title="Remove friend" data-remove="${f.id}">✕</button>
    </div>
  </div>`;
}

// ─── Render requests pane ─────────────────────────────────────────────────
function renderRequests() {
  ...
  pendingIn.forEach(r => {
    ...
    <div class="fp-friend">
      ...
      <div class="fp-friend-actions">
        <button class="fp-action-btn accept" title="Accept" data-accept="${r.friendship_id}" data-uid="${r.id}" data-name="${escHtml(r.name)}">✓</button>
        <button class="fp-action-btn reject" title="Decline" data-decline="${r.friendship_id}">✕</button>
      </div>
    </div>`;
  });
  pane.innerHTML = html;
...
function renderNotifs() {
  ...
  notifs.slice(0, 30).forEach(n => {
    const { icon, title, body } = notifMeta(n);
    html += `
...
function notifMeta(n) {
  ...
  switch (n.type) {
    case "friend_request":
      return { icon: "👋", title: `<b>${escHtml(d.from_name)}</b> sent you a friend request`, body: "" };
    case "friend_accepted":
      return { icon: "🤝", title: `<b>${escHtml(d.from_name)}</b> accepted your friend request`, body: "" };
    case "friend_online":
      return { icon: "🟢", title: `<b>${escHtml(d.name)}</b> is now online`, body: "" };
    case "quick_message":
      return { icon: "💬", title: `<b>${escHtml(d.from_name)}</b>: ${escHtml(d.message)}`, body: "" };
    default:
...
async function handleNotifClick(nid, type) {
  ...
  else if (type === "friend_online" || type === "quick_message" || type === "friend_accepted") {
    switchTab("friends");
    renderFriends();
  }
}
...
async function loadFriends() {
  ...
  const { data: users } = await supabase
    .from("users").select('user_id,"Name"').in("user_id", ids);
...
async function loadPresence() {
  ...
  const { data } = await supabase
    .from("presence")
    .select("user_id, current_game, last_seen")
    .in("user_id", ids);
...
async function sendFriendRequest() {
  ...
  const already = friends.find(f => f.id === target.user_id);
  if (already) { showToast({ icon:"✅", title:"Already friends", body:`You and ${target.Name} are already friends.`, color:"var(--fp-green)" }); return; }
  ...
  if (existing) {
    showToast({ icon:"⏳", title:"Request pending", body:"A friend request already exists.", color:"var(--fp-yellow)" });
    return;
  }
  ...
  showToast({ icon:"📨", title:"Request sent!", body:`Friend request sent to ${target.Name}.`, color:"var(--fp-accent)" });
}
...
function openDmBar(id, name) {
  dmTargetID   = id;
  dmTargetName = name;
  const bar = document.getElementById("fp-dm-bar");
  const lbl = document.getElementById("fp-dm-target-label");
  lbl.textContent = `To: ${name}`;
...
function initAbly() {
  // reuse existing Ably instance if present on page, otherwise create own
  ablyClient = new Ably.Realtime({ key: ABLY_KEY, clientId: myID });

  myChannel = ablyClient.channels.get(`presence:${myID}`);
  myChannel.subscribe(msg => handleAblyMessage(msg.data));

  ablyClient.connection.once("connected", () => {
    // publish "online" to all friends
    announceOnline();
  });
}

function ablyNotify(targetUserID, payload) {
  const ch = ablyClient.channels.get(`presence:${targetUserID}`);
  ch.publish("notify", payload);
}

async function announceOnline() {
  if (!friends.length) {
    // friends may not be loaded yet; load silently then announce
    await loadFriends();
  }
  ...
  switch (data.type) {
    ...
    case "friend_online":
      ...
      presence[data.from_id] = {
        ...
        current_game: data.current_game || "Online",
        last_seen: new Date().toISOString()
      };
      if (panelOpen) renderFriends();
      showToast({
        icon: "🟢",
        title: `${escHtml(data.name)} is online`,
        body: data.current_game || "Just joined DD Games",
        color: "var(--fp-green)",
        onClick: () => { if (!panelOpen) { panelOpen = true; panel.classList.add("open"); } switchTab("friends"); }
      });
      break;
    ...
    case "friend_request":
      ...
      showToast({
        ...
        body: `${escHtml(data.from_name)} wants to be friends.`,
        color: "var(--fp-accent)",
        onClick: () => { if (!panelOpen) { panelOpen = true; panel.classList.add("open"); } switchTab("requests"); }
      });
      break;

    case "friend_accepted":
      loadFriends().then(() => { renderFriends(); loadPresence().then(() => renderFriends()); });
      showToast({
        icon: "🤝",
        title: "Friend accepted!",
        body: `${escHtml(data.from_name)} accepted your request.`,
        color: "var(--fp-green)",
        onClick: () => { if (!panelOpen) { panelOpen = true; panel.classList.add("open"); } switchTab("friends"); }
      });
      break;
    ...
    case "quick_message":
      ...
      showToast({
        icon: "💬",
        title: escHtml(data.from_name),
        body: escHtml(data.message),
        color: "var(--fp-accent2)",
        onClick: () => { if (!panelOpen) { panelOpen = true; panel.classList.add("open"); } switchTab("notifs"); }
      });
      break;
  }
}

// ─── Presence heartbeat (every 90s) ──────────────────────────────────────
setInterval(async () => {
  await supabase.from("presence").upsert({
    user_id: myID,
    current_game: currentGame,
